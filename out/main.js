...
// commands/inProgressIssues.js
var import_vscode = __toESM(require("vscode"));

// src/keys.js
var globalStateKeys = {
  URL: "jinc-url",
  USERNAME: "jinc-username",
  ENABLECOOKIES: "jinc-enable-cookies",
...
// commands/inProgressIssues.js
var issueCommand = (context) => import_vscode.default.commands.registerCommand("jinc.inProgressIssues", function() {
  return __async(this, null, function* () {
    ...
    try {
      ...
      const pickedOption = yield import_vscode.default.window.showQuickPick(
        issues.map((issue) => `${issue.key}: ${issue.name}`)
      );
      const pickedOptionKey = pickedOption.split(":")[0];
      import_vscode.default.env.clipboard.writeText(`${pickedOptionKey}: `);
    } catch (error) {
      console.error(error);
      import_vscode.default.window.showInformationMessage(error);
    }
  });
});
...
var setupCommand = (context) => import_vscode2.default.commands.registerCommand("jinc.setup", function() {
  return __async(this, null, function* () {
    const url = yield import_vscode2.default.window.showInputBox({
      prompt: "Jira URL?"
    });
    const apiUser = yield import_vscode2.default.window.showInputBox({
      prompt: "Jira Api user"
    });
    const apiPassword = yield import_vscode2.default.window.showInputBox({
      prompt: "Jira Api password? (Stored using vscode secrets)",
      password: true
    });
    const username = yield import_vscode2.default.window.showInputBox({
    ...
    yield secrets.store(globalStateKeys.API_PASSWORD, apiPassword);
    import_vscode2.default.window.showInformationMessage(
...
function sayHello() {
  import_vscode3.default.window.showInformationMessage("Hello World!");
}
function deactivate() {
}
module.exports = {
